function [V, Kp] = MaxBellmanE(Par,be,bu,Grid)
...
D = min(f(Par,Grid.KK,Grid.ZZ) , Grid.K(end)); % -1e-3 so we always have positve consumption.

B = p*A+(1-p)*D;
C = (1-p)*A + p * D;


fB = BellmanE(Par,be,bu,Grid.KK,Grid.ZZ,B);
fC = BellmanE(Par,be,bu,Grid.KK,Grid.ZZ,C);
...
for it_inner = 1:MAXIT
    ...
    fC(I) = fB(I);
    B(I) = p*C(I) + (1-p)*A(I);
    fB(I) = BellmanE(Par,be,bu,Grid.KK(I),Grid.ZZ(I),B(I));

    A(~I) = B(~I);
    B(~I) = C(~I);
    fB(~I) = fC(~I);
    C(~I) = p*B(~I) + (1-p)*D(~I);
    fC(~I) = BellmanE(Par,be,bu,Grid.KK(~I),Grid.ZZ(~I),C(~I));
...
% value function.
V = BellmanE(Par,be,bu,Grid.KK,Grid.ZZ,Kp);
